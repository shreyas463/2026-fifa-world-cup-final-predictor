  ...
  return (
    <div>
      ...
      <div className="mt-6 card p-6">
        <h2 className="mb-1 text-lg font-semibold text-white">Group standings</h2>
        <p className="mb-4 text-xs text-slate-400">Real final tables — points and goal difference from the recorded group stage.</p>
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className={r.qualified ? "text-white" : "text-slate-500"}>
                      <td className="py-1">
                        {r.flag} <span className="truncate">{r.name}</span>
                        {r.qualified && (
                          <span className="ml-1 chip !px-1 !py-0 text-[9px]">{r.position === "3" ? "3rd" : r.position}</span>
                        )}
                      </td>
                      <td className="py-1 text-right font-mono">{r.points}</td>
                      <td className="w-7 py-1 text-right font-mono text-slate-500">
        ...
        <p className="mt-3 text-xs text-slate-500">W = winner · R = runner-up · 3rd = best-third qualifier.</p>
      </div>
